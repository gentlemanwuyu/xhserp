                        ...
                        <tr>
                            ...
                            <td>
                                @foreach($user->roles as $role)
                                    @if(\Auth::user()->hasPermissionTo('role_detail'))
                                        <a lay-href="{{route('index::role.detail', ['role_id' => $role->id])}}" lay-text="角色详情[{{$role->id}}]"><span class="layui-badge layui-bg-green">{{$role->display_name}}</span></a>
                                    @else
        ...
        <div class="erp-detail-content">
            <table class="layui-table">
                ...
                @foreach($permissions as $p1)
                    ...
                    ?>
                    <tr>
                        <td rowspan="{{$p1_rowspan}}" @if(in_array($p1['name'], $user_permissions)) class="has-permission" @endif>{{$p1['display_name']}}</td>
                        ...
                        @else
                            ...
                            ?>
                            <td @if(in_array($p1['name'], $user_permissions)) class="has-permission" @endif>{{$first_p2['display_name']}}</td>
                            <td>
                                @if(!empty($first_p2['children']))
                                    @foreach($first_p2['children'] as $p3)
                                        @if(in_array($p3['name'], $user_permissions))
                    ...
                    @foreach($p2s as $p2)
                        <tr>
                            <td @if(in_array($p1['name'], $user_permissions)) class="has-permission" @endif>{{$p2['display_name']}}</td>
                            <td>
