        ...
        <div class="layui-row layui-col-space15">
            ...
            <div class="layui-col-xs2">
                <select name="status">
                    ...
                    @foreach(\App\Modules\Purchase\Models\PurchaseReturnOrder::$statuses as $status_id => $status_name)
                    ...
                    [
                        ...
                        {field: 'total_amount', title: '总金额', width: 100, align: 'center', fixed: 'left', templet: function (d) {
                            ...
                            d.items.forEach(function (item, key) {
                                total_amount += item.quantity * item.purchase_order_item.price;
                            });
                            return total_amount;
                        }},
                        ...
                        {field: 'detail', title: '退货明细', width: 750, align: 'center', templet: function (d) {
                            ...
                            d.items.forEach(function (item, key) {
                                ...
                                var amount = item.quantity * item.purchase_order_item.price;
                                ...
                                html += '<li class="erp-table-list-li" style="width: 100px;">' + item.purchase_order_item.price + '</li>';
                    ...
                    dropdown(res.data,function(data) {
                        ...
                        if (1 == data.status) {
                            ...
                            actions.push({
                                title: "删除",
                                event: function() {
                                    layer.confirm("确认要删除该采购退货单？", {icon: 3, title: "确认"}, function (index) {
                                        layer.close(index);
                                        var load_index = layer.load();
                                        $.ajax({
                                            method: "post",
                                            url: "{{route('purchase::returnOrder.delete')}}",
                                            data: {purchase_return_order_id: data.id},
                                            success: function (data) {
                                                layer.close(load_index);
                                                if ('success' == data.status) {
                                                    layer.msg("采购退货单删除成功", {icon: 1, time: 2000}, function () {
                                                        tableIns.reload();
                                                    });
                                                } else {
                                                    layer.msg("采购退货单删除失败:"+data.msg, {icon: 2, time: 2000});
                                                    return false;
                                                }
                                            },
